...
`bun run test:e2e` needs a Postgres 13+ role with `CREATEDB` in `DATABASE_URL`, e.g. `DATABASE_URL=postgres://localhost:5432/postgres bun run test:e2e`. Each suite creates and drops its own database, and suites skip when `DATABASE_URL` is unset.
